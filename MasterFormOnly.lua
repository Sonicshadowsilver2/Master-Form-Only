...
function GetVersion() --Define anchor addresses
  if GAME_ID == 0x431219CC and ENGINE_TYPE == 'BACKEND' then --PC
    if ReadString(0x09A92F0,4) == 'KH2J' then --EGS v1.0.0.9
			GameVersion = 2
			print('EGS v1.0.0.9 Detected - MasterFormOnly')
			Now  = 0x0716DF8
			Save = 0x09A92F0
			Drive = 0x2A23188
			UCM = 0x2A71998
		...
		elseif ReadString(0x09A9330,4) == 'KH2J' then --EGS v1.0.0.10
			GameVersion = 3
			print('EGS v1.0.0.10 Detected - MasterFormOnly')
			Now  = 0x0716DF8
			Save = 0x09A9330
			Drive = 0x2A231C8
			UCM = 0x2A719D8
		...
		elseif ReadString(0x09A9830,4) == 'KH2J' then --Steam Global v1.0.0.1
			GameVersion = 4
			print('Steam GL v1.0.0.1 Detected - MasterFormOnly')
			Now  = 0x0717008
			Save = 0x09A9830
			Drive = 0x2A236C8
			UCM = 0x2A71ED8
		...
		elseif ReadString(0x09A8830,4) == 'KH2J' then --Steam JP v1.0.0.1
			GameVersion = 5
			print('Steam JP v1.0.0.1 Detected - MasterFormOnly')
			Now  = 0x0716008
			Save = 0x09A8830
			Drive = 0x2A226C8
			UCM = 0x2A70ED8
		...
		elseif ReadString(0x09A98B0,4) == 'KH2J' then --Steam v1.0.0.2
			GameVersion = 6
			print('Steam v1.0.0.2 Detected - MasterFormOnly')
			Now  = 0x0717008
			Save = 0x09A98B0
			Drive = 0x2A23748
			UCM = 0x2A71F58
	...
	if canExecute == true then
		...
		if ReadByte(Now+0x00) == 0x0A then --Pride Lands
			if ReadByte(Now+0x01) == 0x0F then --Groundshaker
				WriteShort(UCM+0x06E8, 0x028A) --Master Form -> Lion Sora
			else
				WriteShort(UCM+0x06E8, 0x0057) --Lion Sora -> Master Form
			end
		end
		if ReadShort(Now+0x00) == 0x0E12 then --Luxord
			if ReadByte(Now+0x08) == 0x3A or ReadByte(Now+0x08) == 0x65 then
				...
				if ReadInt(Drive+0x04) < 0x44160001 and ReadInt(Drive+0x08) > 0x45000000 then
